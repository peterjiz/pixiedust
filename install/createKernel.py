...
class PixiedustInstall(InstallKernelSpec):
    ...
    def parse_command_line(self, argv):
        ...
        if silent:
            ...
            while True:
                ...
                if not os.path.exists(self.spark_home):
                    ...
                    existingInstalls = [d for d in os.listdir(self.spark_home) if
                                        os.path.exists(os.path.join(self.spark_home, d, 'bin', 'pyspark'))]
                    if len(existingInstalls) == 0:
                        download = self.confirm(
                            "Directory {0} does not contain a valid SPARK install".format(self.spark_home),
                            "Download Spark")
                        if download == 'y':
                            download_spark = True
                            break
        ...
        try:
            ...
            print(
                "Error downloading Cloudant Jar: {}. Install will continue without the spark Cloudant connector".format(
                    e))
            ...
            while True:
                ...
                if not os.path.exists(self.scala_home):
                    ...
                    if not installed_scala_version or (
                            str(installed_scala_version[0]) + '.' + str(installed_scala_version[1])) != scala_version:
                        print("A different version of Scala {0} is already installed in this directory.".format(
                            installed_scala_version))
                        continue
                    else:
                        break
    ...
    def downloadPackage(self, packageId):
        if packageId.startswith("http://") or packageId.startswith("https://") or packageId.startswith("file://"):
            ...
            for base in bases:
                ...
                if parts[2] == '0':
                    # resolve version
                    metadataUrl = "{0}/{1}/maven-metadata.xml".format(base,
                                                                      "/".join([parts[0].replace(".", "/"), parts[1]]))
    ...
    def downloadIntroNotebooks(self):
        ...
        introNotebooksUrls = [
            "https://github.com/ibm-watson-data-lab/pixiedust/raw/master/notebook/" + n for n in notebookNames if
            n != ""
        ]
    ...
    def download_spark(self, silent, silent_spark_version):
        while True:
            ...
            spark_download_versions_str = ', '.join(self.spark_download_versions) + ' [{}]'.format(
        ...
        overrides = {
            ...
            "env": {
                ...
                "PYSPARK_SUBMIT_ARGS": "--jars {0} --driver-class-path {1} --master local[10] pyspark-shell".format(
                    ...
                    ":".join(
                        [x for x in [self.pixiedust_home + "/data/libs/*", self.sparkCSVPath, self.commonsCSVPath] if
                         x is not None]),
                ),
